...
function saveApiKey() {
    ...
    if (apiKey) {
        ...
        openModal('error', 'Prosím zadejte platný API klíč');
    }
}
...
async function verifyApiKey(apiKey) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            openModal('error', 'Neplatný API klíč. Zkuste to znovu');
            return false;
        }
    } catch (error) {
        console.error('Chyba ověření API klíče:', error);
        localStorage.removeItem('googleAIStudioApiKey');
        openModal('error', 'Nepodařilo se ověřit API klíč. Zkontrolujte připojení k internetu.');
        return false;
    }
}
...
function generateQuizPrompt(content, quizType, questionCount, difficulty, language) {
    ...
    const languagePrompts = {
        user: 'Všechny otázky a odpovědi piš v českém jazyce.',
        file: `Jazyk VŠECH otázek a VŠECH odpovědí ${quizType !== 'open' ? '(včetně možností A-D)' : ''} MUSÍ BÝT SHODNÝ s dominantním jazykem poskytnutého textu níže.`,
    };

    const difficultySettings = difficultyPrompts[difficulty];
    const languageSettings = languagePrompts[language];
    let promptTemplate = '';

    if (quizType === 'multiple') {
        promptTemplate = `${difficultySettings.guide} ${difficultySettings.complexity} Vygeneruj ${questionCount} ${difficulty} otázek s 4 možnostmi A-D. Formátuj každou otázku přesně takto:
            Otázka: [text otázky]
            A: [možnost A]
            B: [možnost B]
            C: [možnost C]
            ...
            EXTRÉMĚ DŮLEŽITÉ: ${languageSettings}
...
async function generateQuiz() {
    ...
    const language = document.querySelector('.selector-btn[data-language].selected').dataset.language;

    try {
        if (!apiKey) {
            openModal('error', 'Prosím zadejte API klíč');
            return;
        } else if (!file) {
            openModal('error', 'Prosím nahrajte soubor');
            return;
        }

        const fileContent = await extractTextFromPDF(file);
        const prompt = generateQuizPrompt(fileContent, quizType, questionCount, difficulty, language);
        ...
        openModal('error', `Nepodařilo se vygenerovat kvíz: ${error.message}`);
    } finally {
...
async function submitQuiz() {
    ...
    try {
        ...
        openModal('error', 'Nastala chyba při odevzdávání kvízu');
    } finally {
...
function restoreQuizState() {
    ...
    if (savedState) {
        const { difficulty, quizType, questionCount, language } = JSON.parse(savedState);
        ...
        // Restore language
        const languageButtons = document.querySelectorAll('.selector-btn[data-language]');
    ...
    fileInputWrapper.addEventListener('drop', (e) => {
        ...
        if (e.dataTransfer.files.length > 0) {
            ...
            if (file.type === 'application/pdf') {
                ...
                openModal('error', 'Prosím nahrajte PDF soubor');
            }
        }
    });
...
async function openModal(modalName, content = null) {
    const modal = document.querySelector(`[data-modal="${modalName}"]`);
    const currentModal = document.querySelector('.modal.active');
    
    if (currentModal) {
        modalStack.push(currentModal.dataset.modal);
        await closeModal(currentModal.dataset.modal, true);
    }
...
async function closeModal(modalName, force) {
    const modal = document.querySelector(`[data-modal="${modalName}"]`);
    modal.classList.add('closing');
    modal.classList.remove('active');

    setTimeout(() => {
        if (!force && modalStack.length > 0) {
            const previousModalName = modalStack.pop();
            openModal(previousModalName);
        }
