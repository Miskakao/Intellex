...
function generateQuizPrompt(content, quizType, questionCount, difficulty, language) {
    ...
    const languagePrompts = {
        user: 'Všechny texty piš v českém jazyce.',
        file: 'Všechny texty piš ve stejném jazyce, ve kterém je napsaný poskytnutý text. Pokud poskytnutý kombinuje více jazyků použij dominantní jazyk.',
    };
    ...
    if (quizType === 'multiple') {
        promptTemplate = `${difficultySettings.guide} ${difficultySettings.complexity} ${languageSettings} Vygeneruj ${questionCount} ${difficulty} otázek s 4 možnostmi A-D. Formátuj každou otázku přesně takto:
            Otázka: [text otázky]
            A: [možnost A]
            B: [možnost B]
            C: [možnost C]
        ...
        promptTemplate = `${difficultySettings.guide} ${difficultySettings.complexity} ${languageSettings} Vygeneruj ${questionCount} ${difficulty} otázek s volnou odpovědí. Formátuj každou otázku přesně takto:
            Otázka: [text otázky]
        ...
        promptTemplate = `${difficultySettings.guide} ${difficultySettings.complexity} ${languageSettings} Vygeneruj ${questionCount} ${difficulty} otázek buď s 4 možnostmi A-D, nebo s volnou odpovědí. Pro krátké a stručné odpovědi použij možnosti A-D, pro rozsáhlejší a složitější odpovědi použji vlnou odpověď, ale vždy musí být od obou typů otázek minimálně jedna.
